...
export function AnalyzingImage({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      role="status"
      aria-label="Analyzing image"
      className={cx("relative isolate shrink-0", className)}
      {...props}
    >
      <motion.div
...
/** Ligne d'indicateur "l'IA analyse une image" avec label animé. */
export function AnalyzingImageIndicator({ label = "Analyzing image" }: { label?: string }) {
  return (
    <div className="flex items-center gap-2 mb-1.5" style={{ color: "var(--blue-accent)" }}>
      <AnalyzingImage style={{ width: 22, height: 22 }} />
      <span className="an-think-shimmer text-[13px] font-medium" style={{ color: "var(--text-primary)" }}>
        {label}
      </span>
    </div>
  );
